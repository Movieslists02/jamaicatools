...
function DocumentWorkspace({
  ...
  headerTitle = "📄 Document Tool",
  headerSubtitle = "Upload your document to begin.",
  actionLabel = "Process Document",
  uploadTitle = "Drag and drop your document here",
  uploadDescription = "Upload one supported document up to 25 MB.",
  browseLabel = "Browse Document",
  filesTitle = "Selected Document",
  ...
  const isAcceptedFile = (file) => {
    ...
    const hasAcceptedType =
      file.type && acceptedTypes.includes(file.type);

    const hasAcceptedExtension = acceptedExtensions.some(
  ...
  const handleFiles = (fileList) => {
    ...
    if (invalidFile) {
      setError(`Please select a valid ${fileTypeLabel} only.`);
      return;
    }
    ...
    if (oversizedFile) {
      setError(
        `Each document must be smaller than ${formatFileSize(maxFileSize)}.`,
      );
      return;
    }
  ...
  const handleSubmit = (event) => {
    ...
    if (selectedFiles.length < minimumFiles) {
      setError(
        minimumFiles === 1
          ? `Please select a ${fileTypeLabel} before continuing.`
          : `Please select at least ${minimumFiles} documents before continuing.`,
      );
      return;
    }
